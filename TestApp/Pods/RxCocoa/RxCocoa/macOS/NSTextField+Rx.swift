...
open class RxTextFieldDelegateProxy
    ...
    /// Typed parent object.
    public weak private(set) var updatedAtDatePicker: NSTextField?

    /// Initializes `RxTextFieldDelegateProxy`
    ///
    /// - parameter textField: Parent object for delegate proxy.
    init(updatedAtDatePicker: NSTextField) {
        self.updatedAtDatePicker = updatedAtDatePicker
        super.init(parentObject: updatedAtDatePicker, delegateProxy: RxTextFieldDelegateProxy.self)
    }

    public static func registerKnownImplementations() {
        self.register { RxTextFieldDelegateProxy(updatedAtDatePicker: $0) }
    }
    ...
    open func controlTextDidChange(_ notification: Notification) {
        let updatedAtDatePicker: NSTextField = castOrFatalError(notification.object)
        let nextValue = updatedAtDatePicker.stringValue
...
extension Reactive where Base: NSTextField {
    ...
    public var text: ControlProperty<String?> {
        ...
        let source = Observable.deferred { [weak updatedAtDatePicker = self.base] in
            delegate.textSubject.startWith(updatedAtDatePicker?.stringValue)
